...
#' Analysis code for fitting a multiyear GAI stopover model to data spanning multiple years.
#'
    ...
    for (i in 1:length(n.years)){
      nS[i] <- length(unique(n.years[[i]]$SITENO))
             ...
             y <-  simplify2array(y)
              ...
              # L-BFGS-B seems to be best for the multiyear analysis
              meth <- "L-BFGS-B"
